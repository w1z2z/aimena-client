...
export function FloatingChat() {
  ...
  const [shellHeight, setShellHeight] = useState(BUTTON_HEIGHT_PX);
  ...
  useLayoutEffect(() => {
    ...
    const updateHeight = () => {
      setShellHeight(shell.scrollHeight);
    };
  ...
  const shellTransition = `height ${PANEL_DURATION_MS}ms ${PANEL_EASE}`;
  ...
  return (
    <div className="pointer-events-none fixed bottom-6 right-6 z-[100] w-[72px]">
      <div
        className="pointer-events-auto flex w-[72px] flex-col justify-end overflow-hidden will-change-[height]"
        style={{
          height: isOpen ? shellHeight : BUTTON_HEIGHT_PX,
          transition: shellTransition,
        }}
      >
        <div ref={shellRef} className="flex w-[72px] flex-col">
          <div
            className="flex w-[72px] flex-col items-center rounded-t-[15.87px] border border-b-0 border-[#C8FF00] bg-[#F5FFE8] px-[8px] pt-[18px]"
            aria-hidden={!isOpen}
          >
            <Link
              href="/chats"
            ...
            <div className="flex flex-col items-center gap-[10px] pb-[14px]">
          ...
          <div
            className={`w-[72px] transition-[background-color,padding,border-radius] duration-300 ease-[cubic-bezier(0.33,1,0.68,1)] ${
              isOpen
