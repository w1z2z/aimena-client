import type { IconProps } from "./types";

export function DeleteIcon({ className, ...props }: IconProps) {
  return (
    <svg viewBox="0 0 24 26" fill="none" aria-hidden className={className} {...props}>
      <path
        d="M20.5491 5.14648L19.8149 17.024C19.6272 20.0586 19.5335 21.576 18.7729 22.6669C18.3967 23.2062 17.9126 23.6614 17.351 24.0035C16.2153 24.6954 14.6951 24.6954 11.6546 24.6954C8.61017 24.6954 7.08794 24.6954 5.95144 24.0022C5.38955 23.6595 4.90521 23.2035 4.52926 22.6633C3.76886 21.5707 3.67713 20.0512 3.49368 17.0123L2.77734 5.14648"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
      />
      <path
        d="M14.625 18.179V11.0703"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
      />
      <path
        d="M8.70117 18.179V11.0703"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
      />
      <path
        d="M1 5.14674H22.3261M16.4682 5.14674L15.6594 3.47825C15.1221 2.36992 14.8534 1.81575 14.3901 1.47013C14.2873 1.39347 14.1785 1.32527 14.0646 1.26622C13.5515 1 12.9356 1 11.7039 1C10.4413 1 9.81004 1 9.28836 1.27738C9.17275 1.33886 9.06242 1.40982 8.95853 1.48952C8.48977 1.84913 8.22792 2.42358 7.70422 3.57247L6.98661 5.14674"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
      />
    </svg>
  );
}
